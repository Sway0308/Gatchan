...
namespace Elementary.Core
{
    ...
    public class StrFuncCore
    {
        ...
        {
            if (value is string)
                value = BaseFuncCore.CStr(value).Trim();
        }
        ...
        {
            return StrIsEmpty(BaseFuncCore.CStr(s));
        }
        ...
        public static bool StrIsNotEmpty(params string[] args)
        {
            ...
            {
                // 任何一個成員為空字串則傳回 false
                if (StrFuncCore.StrIsEmpty(s)) { return false; }
            }
            return true;
        }
        ...
        public static string StrRight(string s, int length)
        {
            ...
            //在字串中擷取指定起始位置後子字串
            return StrFuncCore.StrSubstring(s, iStartIndex);
        }
        ...
        {
            return StrFuncCore.SameText(BaseFuncCore.CStr(s1), BaseFuncCore.CStr(s2));
        }
        ...
        {
            return SameTextOr(s, StrFuncCore.StrSplit(args, ","));
        }
        ...
        {
            if (StrFuncCore.StrIsNotEmpty(s1))
                s1 = s1 + delimiter;
            return s1 + s2;
        }
